/// @DnDAction : YoYo Games.Common.If_Variable
/// @DnDVersion : 1
/// @DnDHash : 12355266
/// @DnDArgument : "var" "global.InventoryLeftTabSelected"
/// @DnDArgument : "value" "1"
if(global.InventoryLeftTabSelected == 1)
{
	/// @DnDAction : YoYo Games.Common.If_Variable
	/// @DnDVersion : 1
	/// @DnDHash : 2528B0BE
	/// @DnDParent : 12355266
	/// @DnDArgument : "var" "global.InventoryOpen"
	/// @DnDArgument : "value" "1"
	if(global.InventoryOpen == 1)
	{
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 622DF559
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(0)) + 467 -72 "
		/// @DnDArgument : "y" "125"
		/// @DnDArgument : "sprite" "global.InventorySlot1"
		draw_sprite(global.InventorySlot1, 0, (144*(0)) + 467 -72 , 125);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 7FD67ADE
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(1)) + 467 -72 "
		/// @DnDArgument : "y" "125"
		/// @DnDArgument : "sprite" "global.InventorySlot2"
		draw_sprite(global.InventorySlot2, 0, (144*(1)) + 467 -72 , 125);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 638DAB3A
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(2)) + 467 -72 "
		/// @DnDArgument : "y" "125"
		/// @DnDArgument : "sprite" "global.InventorySlot3"
		draw_sprite(global.InventorySlot3, 0, (144*(2)) + 467 -72 , 125);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 30B24E35
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(3)) + 467 -72 "
		/// @DnDArgument : "y" "125"
		/// @DnDArgument : "sprite" "global.InventorySlot4"
		draw_sprite(global.InventorySlot4, 0, (144*(3)) + 467 -72 , 125);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 7B5A7BE1
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(4)) + 467 -72 "
		/// @DnDArgument : "y" "125"
		/// @DnDArgument : "sprite" "global.InventorySlot5"
		draw_sprite(global.InventorySlot5, 0, (144*(4)) + 467 -72 , 125);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 62D137E8
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(0)) + 467 -72 "
		/// @DnDArgument : "y" "269"
		/// @DnDArgument : "sprite" "global.InventorySlot6"
		draw_sprite(global.InventorySlot6, 0, (144*(0)) + 467 -72 , 269);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 6FD87978
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(1)) + 467 -72 "
		/// @DnDArgument : "y" "269"
		/// @DnDArgument : "sprite" "global.InventorySlot7"
		draw_sprite(global.InventorySlot7, 0, (144*(1)) + 467 -72 , 269);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 33449983
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(2)) + 467 -72 "
		/// @DnDArgument : "y" "269"
		/// @DnDArgument : "sprite" "global.InventorySlot8"
		draw_sprite(global.InventorySlot8, 0, (144*(2)) + 467 -72 , 269);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 072EFAD0
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(3)) + 467 -72 "
		/// @DnDArgument : "y" "269"
		/// @DnDArgument : "sprite" "global.InventorySlot9"
		draw_sprite(global.InventorySlot9, 0, (144*(3)) + 467 -72 , 269);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 070A7D76
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(4)) + 467 -72 "
		/// @DnDArgument : "y" "269"
		/// @DnDArgument : "sprite" "global.InventorySlot10"
		draw_sprite(global.InventorySlot10, 0, (144*(4)) + 467 -72 , 269);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 78323A46
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(0)) + 467 -72 "
		/// @DnDArgument : "y" "413"
		/// @DnDArgument : "sprite" "global.InventorySlot11"
		draw_sprite(global.InventorySlot11, 0, (144*(0)) + 467 -72 , 413);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 332131A7
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(1)) + 467 -72 "
		/// @DnDArgument : "y" "413"
		/// @DnDArgument : "sprite" "global.InventorySlot12"
		draw_sprite(global.InventorySlot12, 0, (144*(1)) + 467 -72 , 413);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 67A2550B
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(2)) + 467 -72 "
		/// @DnDArgument : "y" "413"
		/// @DnDArgument : "sprite" "global.InventorySlot13"
		draw_sprite(global.InventorySlot13, 0, (144*(2)) + 467 -72 , 413);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 756B4EEF
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(3)) + 467 -72 "
		/// @DnDArgument : "y" "413"
		/// @DnDArgument : "sprite" "global.InventorySlot14"
		draw_sprite(global.InventorySlot14, 0, (144*(3)) + 467 -72 , 413);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 1565A2B9
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(4)) + 467 -72 "
		/// @DnDArgument : "y" "413"
		/// @DnDArgument : "sprite" "global.InventorySlot15"
		draw_sprite(global.InventorySlot15, 0, (144*(4)) + 467 -72 , 413);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 1D39F253
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(0)) + 467 -72 "
		/// @DnDArgument : "y" "557"
		/// @DnDArgument : "sprite" "global.InventorySlot16"
		draw_sprite(global.InventorySlot16, 0, (144*(0)) + 467 -72 , 557);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 1D777D67
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(1)) + 467 -72 "
		/// @DnDArgument : "y" "557"
		/// @DnDArgument : "sprite" "global.InventorySlot17"
		draw_sprite(global.InventorySlot17, 0, (144*(1)) + 467 -72 , 557);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 1F9529B7
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(2)) + 467 -72 "
		/// @DnDArgument : "y" "557"
		/// @DnDArgument : "sprite" "global.InventorySlot18"
		draw_sprite(global.InventorySlot18, 0, (144*(2)) + 467 -72 , 557);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 6862B298
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(3)) + 467 -72 "
		/// @DnDArgument : "y" "557"
		/// @DnDArgument : "sprite" "global.InventorySlot19"
		draw_sprite(global.InventorySlot19, 0, (144*(3)) + 467 -72 , 557);
	
		/// @DnDAction : YoYo Games.Drawing.Draw_Sprite
		/// @DnDVersion : 1
		/// @DnDHash : 06034FC3
		/// @DnDParent : 2528B0BE
		/// @DnDArgument : "x" "(144*(4)) + 467 -72 "
		/// @DnDArgument : "y" "557"
		/// @DnDArgument : "sprite" "global.InventorySlot20"
		draw_sprite(global.InventorySlot20, 0, (144*(4)) + 467 -72 , 557);
	}
}